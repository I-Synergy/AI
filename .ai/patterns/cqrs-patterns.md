...
7. Entities are used **internally** for domain logic and persistence — never passed across layer boundaries
8. Each operation gets its own **subfolder** with separate files for Command/Query, Handler, and Response
9. Handler classes are named with `CommandHandler` / `QueryHandler` suffix
    ...
    : IQueryHandler<Get{Entity}ByIdQuery, Get{Entity}ByIdResponse>
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // TODO: map entity to {Entity} model
...
// Read single — named DbSet FirstOrDefaultAsync + map to model
var entity = await dataContext.{Entities}.FirstOrDefaultAsync(e => e.{Entity}Id == id, cancellationToken);
// TODO: map entity to {Entity} model
var model = /* map entity */;

// Read list — named DbSet + Select projection
var models = await dataContext.{Entities}
    .OrderBy(e => e.Description)
    .Select(e => /* map e to {Entity} model */)
    .ToListAsync(cancellationToken);
...
public async Task<GetBudgetByIdResponse> HandleAsync(...)
{
    ...
    // TODO: map entity to Budget model
    var budget = /* map entity */;
